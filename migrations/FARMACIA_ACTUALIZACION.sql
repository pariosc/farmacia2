...
-- Actualización final incremental de Farmacia e Inventario.
-- Requiere que el script base 001-003 ya haya sido ejecutado.
-- Ejecutar una sola vez con psql; crear respaldo antes.
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION 'Faltan tablas base o el script anterior no fue aplicado. No se modificó nada.';
    END IF;
END $$;

-- Las secciones siguientes son transaccionales e idempotentes.

-- ===== 005: origen alternativo de consumo =====
\set ON_ERROR_STOP on
