(() => {
  ...
  const usuarioInput = $("consumoUsuario");
  const usuarioAyuda = $("consumoUsuarioAyuda");
  if (usuarioSesion?.id_usuario) {
    usuarioInput.value = String(usuarioSesion.id_usuario);
    usuarioInput.readOnly = true;
    usuarioAyuda.textContent = `Asignado por login: ${usuarioSesion.username} (#${usuarioSesion.id_usuario}).`;
  } else {
    usuarioAyuda.textContent = "Seguridad aún no devuelve id_usuario; usa el ID manual de transición.";
  }
  const usuarioInicial = Number(usuarioInput.value) || 1;
